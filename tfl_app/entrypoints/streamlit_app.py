...
import tfl_app.ui.fragments.map_fragments as _map_fragments
...
@_safe_page('About')
def _page_about():
    _run_page_renderer("tfl_app.ui.pages.about")


@_safe_page('Media Briefings')
def _page_turn_off_tap():
    _run_page_renderer("tfl_app.ui.pages.multimedia")


@_safe_page('Policy Context')
def _page_solutions():
    _run_page_renderer("tfl_app.ui.pages.solutions")

@_safe_page('Clients')
def _page_client_lookup():
    _run_page_renderer("tfl_app.ui.pages.clients")


@_safe_page('Legislators')
def _page_member_lookup():
    _run_page_renderer("tfl_app.ui.pages.legislators")
...
def _page_map_address_full_pass():
    _run_page_renderer("tfl_app.ui.pages.map_address")
...
def _page_lobby_lookup():
    _run_page_renderer("tfl_app.ui.pages.lobbyists")
...
_session_from_year = _app_runtime._session_from_year
_add_session_from_year = _app_runtime._add_session_from_year
_tfl_session_for_filter = _app_runtime._tfl_session_for_filter
...
def render_app() -> None:
    _active_page, nav_query, nav_search_submitted, nav_suggest_slot = _render_navigation_shell()
    ...
    if nav_suggestions:
        ...
        if nav_pick in nav_suggestion_map:
            ...
            if target == "client":
                ...
                if not _same_page(_active_page, _lobby_page):
                    st.switch_page(_lobby_page)
                    st.stop()
    else:
        nav_suggest_slot.empty()

    if nav_search_submitted and not nav_skip_submit and nav_query:
        ...
        if nav_bundle.bill_query:
            ...
            if not _same_page(target_page, _active_page):
