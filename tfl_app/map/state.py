...
class MapState:
    ...
    all_client_names: tuple[str, ...]
    client_names_by_session: dict[str, tuple[str, ...]]
...
def _client_names_by_session(base: pd.DataFrame) -> dict[str, tuple[str, ...]]:
    if base.empty or "Session" not in base.columns or "Client" not in base.columns or "IsTFL" not in base.columns:
        return {}
    tfl_only = base[base["IsTFL"] == 1].copy()
    if tfl_only.empty:
        return {}
    tfl_only["Session"] = tfl_only["Session"].fillna("").astype(str).str.strip()
    tfl_only["Client"] = tfl_only["Client"].fillna("").astype(str).str.strip()
    tfl_only = tfl_only[(tfl_only["Session"] != "") & (tfl_only["Client"] != "")]
    if tfl_only.empty:
        return {}
    grouped: dict[str, tuple[str, ...]] = {}
    for session_val, piece in tfl_only.groupby("Session", sort=False):
        grouped[str(session_val).strip()] = tuple(sorted(set(piece["Client"].tolist())))
    return grouped
...
def build_client_subdivision_edges(matches: pd.DataFrame) -> pd.DataFrame:
    return _expand_client_subdivision_edges(matches)


def resolve_active_tfl_clients(
    map_state: MapState,
    *,
    scope: str,
    session_for_filter: str | None,
    ...
    if str(scope).strip() == "This Session" and session_for_filter:
        session_key = str(session_for_filter).strip()
        return tuple(map_state.client_names_by_session.get(session_key, ()))
    return tuple(map_state.all_client_names)
...
def build_map_state_from_sources(
    ...
    *,
    ...
    build_client_matches: Callable[[tuple[str, ...], Mapping[str, pd.DataFrame]], pd.DataFrame] | None = None,
) -> MapState:
    ...
    all_client_names = _unique_tfl_entity_names(lobby_tfl_client_all)
    client_names_by_session = _client_names_by_session(lobby_tfl_client_all)
    ...
    client_subdivision_edges_all = pd.DataFrame(columns=_MAP_EDGE_COLUMNS)
    if callable(build_client_matches):
        matches = _dataframe_or_empty(build_client_matches(all_client_names, reference_tables), columns=_MAP_MATCH_COLUMNS)
        client_subdivision_edges_all = build_client_subdivision_edges(matches)
    return MapState(
        path=path,
        ...
        client_names_by_session=client_names_by_session,
...
def _build_hotspot(subdivision_matches: pd.DataFrame) -> tuple[str, float]:
    if subdivision_matches.empty:
        return "--", 0.0
    top_hotspot = subdivision_matches.nlargest(1, ["high_total", "match_count"])
    if top_hotspot.empty:
        return "--", 0.0
    row = top_hotspot.iloc[0]
    return (
        f"{str(row.get('subdivision_type', '')).strip()} - {str(row.get('subdivision_name', '')).strip()}",
        float(row.get("high_total", 0.0) or 0.0),
    )
...
def build_atlas_bundle(
    ...
    session_for_filter: str | None,
    *,
    client_subdivision_edges_all: pd.DataFrame | None = None,
    prepare_overlap_pool: Callable[[pd.DataFrame, str], pd.DataFrame] | None = None,
) -> AtlasBundle:
    del prepare_overlap_pool
    totals = _build_client_totals(
        map_state.lobby_tfl_client_all,
        scope=scope,
        session_for_filter=session_for_filter,
    )
    tfl_spend = totals[totals["IsTFL"] == 1].reset_index(drop=True)
    edges_source = client_subdivision_edges_all
    if edges_source is None:
        edges_source = getattr(map_state, "client_subdivision_edges_all", pd.DataFrame())
    edges_all = _dataframe_or_empty(edges_source, columns=_MAP_EDGE_COLUMNS)
    ...
    hotspot_label, hotspot_high = _build_hotspot(subdivision_matches)
    return AtlasBundle(
