...
def render_page(*, services: AppServices, ctx: dict[str, Any] | None = None) -> None:
    ...
    witness_all_cache: pd.DataFrame | None = None
    bill_status_all_cache: pd.DataFrame | None = None
    lobby_tfl_client_all_cache: pd.DataFrame | None = None

    def _get_witness_all() -> pd.DataFrame:
        nonlocal witness_all_cache
        if witness_all_cache is None:
            witness_all_cache = get_app_table_readonly(str(PATH), "Wit_All")
        return witness_all_cache

    def _get_bill_status_all() -> pd.DataFrame:
        nonlocal bill_status_all_cache
        if bill_status_all_cache is None:
            bill_status_all_cache = get_app_table_readonly(str(PATH), "Bill_Status_All")
        return bill_status_all_cache

    def _get_lobby_tfl_client_all() -> pd.DataFrame:
        nonlocal lobby_tfl_client_all_cache
        if lobby_tfl_client_all_cache is None:
    ...
    if bill_mode:
        ...
        render_bill_search_results(
            ...
            tfl_session_val,
            _get_witness_all(),
            _get_bill_status_all(),
            _get_lobby_tfl_client_all(),
            short_to_names,
        )
