...
def _clone_cache_value(value: Any) -> Any:
    # Map runtime payloads are cached as read-only bundles; keeping shared
    # frame references avoids expensive duplication on every map rerun.
...
def _refresh_map_runtime_context(services: MapServices, ctx: dict[str, Any]) -> dict[str, Any]:
    required = (
        "PATH",
        "get_map_state",
...
def _run_fragment(services: MapServices, storage_key: str) -> None:
    ...
    ctx = _refresh_map_runtime_context(services, ctx)
    ctx.update(_TRANSIENT_CONTEXTS.get(storage_key, {}))
    if hasattr(st, "session_state"):
    ...
    module.render_map_workspace(ctx, services)
