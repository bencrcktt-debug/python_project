...
from tfl_app.ui.fragments.prepared_cache import (
    get_single_prepared_context,
    remember_single_prepared_context,
)
...
_PREPARED_MAP_CONTEXT_CACHE: dict[str, Any] = {}
...
def _remember_prepared_context(runtime_signature: str, ctx: MapWorkspacePreparedContext) -> None:
    remember_single_prepared_context(_PREPARED_MAP_CONTEXT_CACHE, runtime_signature, ctx)


def _prepared_context(runtime_signature: str) -> MapWorkspacePreparedContext | None:
    cached = get_single_prepared_context(_PREPARED_MAP_CONTEXT_CACHE, runtime_signature)
    return cached if isinstance(cached, MapWorkspacePreparedContext) else None
...
def _refresh_map_runtime_context(
    services: MapServices,
    ctx: dict[str, Any] | MapWorkspacePreparedContext,
) -> MapWorkspacePreparedContext:
    payload = dict(getattr(ctx, "payload", ctx) or {})
    required = (
        "PATH",
        "get_map_state",
    ...
    if any(name not in services.values for name in required):
        return MapWorkspacePreparedContext(
            selector=MapWorkspaceSelector.from_payload(payload),
            payload=payload,
        )

    try:
        path = str(services.require("PATH"))
        map_state = services.require("get_map_state")(path)
        tfl_sessions = set(getattr(map_state, "map_sessions", []) or [])
        session_for_filter = services.require("_tfl_session_for_filter")(
            st.session_state.get("map_session"),
            tfl_sessions,
        )
        selected_subdivision_signature = services.require("_map_runtime").build_selected_subdivision_signature(
        ...
        if cached is not None:
            ...
            return MapWorkspacePreparedContext(
                selector=MapWorkspaceSelector(
                    ...
                    forensics_source_signature=str(refreshed.get("_map_forensics_source_signature", "") or "").strip(),
                ),
                payload=refreshed,
            )
        map_forensics_bundle = services.require("get_map_forensics_bundle")(
            path,
            str(st.session_state.get("map_scope", "")),
            session_for_filter,
            selected_subdivision_signature,
        )
        atlas_bundle = services.require("get_map_atlas_bundle")(
            path,
            str(st.session_state.get("map_scope", "")),
            session_for_filter,
        )
    ...
    subdivision_matches = map_forensics_bundle.subdivision_matches
    docket = st.session_state.get("map_watchlist", [])
    docket_count = len(docket) if isinstance(docket, list) else 0
    ...
    if str(payload.get("_map_forensics_source_signature", "")).strip() != forensics_source_signature:
    ...
    refreshed.update(
        {
            ...
            "totals": map_forensics_bundle.totals,
            "tfl_spend": map_forensics_bundle.tfl_spend,
    ...
    prepared = MapWorkspacePreparedContext(
        selector=MapWorkspaceSelector(
            path=path,
            runtime_signature=runtime_signature,
            forensics_source_signature=forensics_source_signature,
        ),
        payload=refreshed,
    )
    _remember_prepared_context(runtime_signature, prepared)
    return prepared


def _run_fragment(services: MapServices, storage_key: str) -> None:
    persisted_selector = MapWorkspaceSelector()
    if hasattr(st, "session_state"):
        raw = st.session_state.get(storage_key, {})
        if isinstance(raw, dict):
            persisted_selector = MapWorkspaceSelector.from_payload(raw)
    payload = dict(persisted_selector.to_payload())
    payload.update(_TRANSIENT_CONTEXTS.get(storage_key, {}))
    ctx = _refresh_map_runtime_context(services, payload)
    refreshed_payload = dict(ctx.payload)
    refreshed_payload.update(_TRANSIENT_CONTEXTS.get(storage_key, {}))
    prepared = MapWorkspacePreparedContext(selector=ctx.selector, payload=refreshed_payload)
    if hasattr(st, "session_state"):
        st.session_state[storage_key] = prepared.selector.to_payload()
    module = importlib.import_module("tfl_app.ui.renderers")
    module.render_map_workspace(prepared, services)


@st.fragment
def render_map_workspace_fragment(services: MapServices, storage_key: str = "_map_workspace_ctx") -> None:
