...
from src.chart_runtime import stable_json_signature
from src.map_geo_runtime import SUBDIVISION_TYPE_COLORS, _hex_to_rgba, _subdivision_color_hex
...
<script>
  require([
    ...
    /* Sketch complete â†’ reverse-geocode the centroid of the drawn area */
    sketch.on("create", (evt) => {{
      ...
      if (badge) badge.textContent = "Area scanned â€” see collected addresses \\u2192";
...
def render_address_overlap_arcgis_map(
    ...
    # PERFORMANCE: Vectorized point_rows builder â€” avoids per-row itertuples + html.escape loop
...
<script>
  ...
  require([
    ...
    /* -- Phase 1: View ready â€” dismiss loading overlay and zoom to graphics -- */
    view.when(() => {{
      ...
      const loadAdvanced = () => {{
        ...
        /* -- Reference boundary layers (deferred â€” not needed for initial render) -- */
        const countyLayer = new FeatureLayer({{
          url: "{TEA_ARCGIS_COUNTY_LAYER_URL}",
        ...
        /* Sketch complete â†’ batch analysis info */
        sketch.on("create", (evt) => {{
          if (evt.state !== "complete") return;
...
<script>
  ...
  require([
    ...
    /* Sketch complete â†’ batch analysis info + address scanning */
    sketch.on("create", (evt) => {{
      if (evt.state !== "complete") return;
      const drawn = evt.graphic.geometry;
    ...
    /* Click â†’ reverse geocode for address collection */
    view.on("click", (evt) => {{
      if (evt.mapPoint) {{
        reverseGeocode(evt.mapPoint.latitude, evt.mapPoint.longitude);
      }}
