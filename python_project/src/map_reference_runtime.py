...
REFERENCE_SNAPSHOT_DIR = Path(
    os.getenv(
        ...
        Path(__file__).resolve().parents[1] / "data" / "_reference_snapshots",
    )
)
...
def _load_or_fetch_reference_snapshot(
    ...
    if not cached.empty:
        return cached
    return _normalize_reference_frame(fetch_remote(), _REFERENCE_SNAPSHOT_COLUMNS[snapshot_key])
...
def refresh_reference_snapshot(
    ...
    *,
    ...
    remote_fetchers: dict[str, Callable[[], pd.DataFrame]] = {
    ...
    frame = _normalize_reference_frame(remote_fetchers[snapshot_key](), _REFERENCE_SNAPSHOT_COLUMNS[snapshot_key])
...
def fetch_tceq_water_district_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "tceq_water_district_centroids",
        _fetch_tceq_water_district_centroids_remote,
    )
...
def fetch_tceq_groundwater_district_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "tceq_groundwater_district_centroids",
        _fetch_tceq_groundwater_district_centroids_remote,
    )
...
def fetch_texas_rma_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "texas_rma_centroids",
        _fetch_texas_rma_centroids_remote,
    )
...
def fetch_texas_junior_college_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "texas_junior_college_centroids",
        _fetch_texas_junior_college_centroids_remote,
    )
...
def fetch_texas_navigation_district_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "texas_navigation_district_centroids",
        _fetch_texas_navigation_district_centroids_remote,
    )
...
def fetch_nctcog_transit_provider_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "nctcog_transit_provider_centroids",
        _fetch_nctcog_transit_provider_centroids_remote,
    )
...
def fetch_txdot_seaport_centroids() -> pd.DataFrame:
    return _load_or_fetch_reference_snapshot(
        "txdot_seaport_centroids",
        _fetch_txdot_seaport_centroids_remote,
    )
