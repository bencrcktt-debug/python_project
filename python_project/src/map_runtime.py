...
def build_map_forensics_bundle(atlas_bundle, *, selected_subdivision_signature: str) -> MapForensicsBundle:
    totals = atlas_bundle.totals.copy()
    tfl_spend = atlas_bundle.tfl_spend.copy()
    subdivision_matches = atlas_bundle.subdivision_matches.copy()
    ...
    top_hotspot = (
        subdivision_matches.sort_values(["high_total", "match_count"], ascending=[False, False]).head(1)
