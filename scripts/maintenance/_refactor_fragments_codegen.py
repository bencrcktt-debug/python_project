...
ROOT = next(parent for parent in pathlib.Path(__file__).resolve().parents if (parent / "tfl_app").exists())
    ...
    refreshed.update(
        {
            ...
            "_atlas_label": f"ðŸ—ºï¸ Coverage Atlas ({atlas_count:,})",
            "_forensics_label": "ðŸ” Address Forensics",
            "_docket_label": (
                f"ðŸ“‹ Case Docket ({docket_count:,})"
                if docket_count
                else "ðŸ“‹ Case Docket"
            ),
        }
